...
public class DisplayData : MonoBehaviour {
	...
	bool socketReady = false;                // global variables are setup here
     ...
     public String Host = "192.168.137.5";
...
/*	if (Attention > 70)
	{
		itemtwo.transform.GetComponent<Renderer>().material.color = Color.yellow;
	}
	if (Meditation > 70)
	{
		itemtwo.transform.GetComponent<Renderer>().material.color = Color.black;
	}
	if (Blink < 50 && Blink >40)
	{
		itemtwo.transform.GetComponent<Renderer>().material.color = Color.blue;
	}
	if (Blink < 70 && Blink >50)
	{
		itemtwo.transform.GetComponent<Renderer>().material.color = Color.green;
	}*/
	if (Blink >100)
	{
 ...
 if (Attention > 70) {
     ...
     itemtwo.transform.GetComponent<Renderer>().material.color = Color.green;
		...
		//itemtwo.transform.GetComponent<Renderer>().material.color = Color.red;
...
//				print(" Click Connect Button Frame");
 ...
 public void LedON () {                    
 writeSocket("1");        // write out a 1                                    // wait for that function to be called before proceeding (prevents a bounce)
 return;
 
 }
 
 public void LedOFF () {
 writeSocket("0");        // write out a 0                                  // wait for that function to be called before proceeding (prevents a bounce)
 return;
 }
